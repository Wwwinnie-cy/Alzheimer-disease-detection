...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model, fold):
        if not os.path.exists('attention_model_V15/saved_checkpoints_V15_layer2'):
            os.makedirs('attention_model_V15/saved_checkpoints_V15_layer2')

        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        model_path = f'attention_model_V15/saved_checkpoints_V15_layer2/checkpoint_fold_{fold}_{timestamp}.pth'
...
log_file = f'V15_patience20_layer2_training_log_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
