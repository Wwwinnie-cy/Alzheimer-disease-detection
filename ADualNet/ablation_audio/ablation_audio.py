...
class AttentionFusionModelWithoutAudio(nn.Module):
    def __init__(self, video_dim, num_heads, num_classes, dropout=0.65):
        super(AttentionFusionModelWithoutAudio, self).__init__()
        ...
        # 新增：位置编码
        self.positional_encoding = nn.Parameter(torch.randn(10, 512))  # assuming 10 time steps
        ...
        self.fc = nn.Sequential(
            nn.Linear(512, 256),  # 不再拼接音频特征，输入改为512
            nn.ReLU(),
            nn.Dropout(dropout),
    ...
    def forward(self, video_features):
        ...
        # 位置编码：对 batch_size 维度进行广播
        positional_encoding = self.positional_encoding.unsqueeze(1).expand(-1, video_features.size(1), -1)
        video_features = video_features + positional_encoding

        # 初始化 self-attention 输出
        ...
        # 特征激活
        video_final = video_self.mean(dim=0)

        # 层归一化
        video_final = self.video_norm(video_final)

        # 分类
        logits = self.fc(video_final)
        return logits
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model, fold):
        if not os.path.exists('ablation_experiment_V9/ablation_audio/saved_checkpoints_V9'):
            os.makedirs('ablation_experiment_V9/ablation_audio/saved_checkpoints_V9')

        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        model_path = f'ablation_experiment_V9/ablation_audio/saved_checkpoints_V9/checkpoint_fold_{fold}_{timestamp}.pth'
...
model = AttentionFusionModelWithoutAudio(video_dim, num_heads, num_classes).to(device)
...
log_file = f'ablation_experiment_V9/ablation_audio/patience30_log_new__{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
...
for fold in range(1, num_folds + 1):
    ...
    fold_path = f'5fold_data_V9/fold_{fold}'
    log_message(f'Processing fold {fold}')
    train_dataset, val_dataset, test_dataset = load_fold_data(fold_path)

    train_loader = DataLoader(train_dataset, batch_size=8, shuffle=True, num_workers=32, pin_memory=True)
    val_loader = DataLoader(val_dataset, batch_size=8, shuffle=False, num_workers=32, pin_memory=True)
    test_loader = DataLoader(test_dataset, batch_size=8, shuffle=False, num_workers=32, pin_memory=True)

    # 计算类别权重
    train_labels = [label.item() for _, _, label in train_dataset]  # 提取训练集中的所有标签
    class_counts = np.bincount(train_labels)  # 计算每个类别的样本数量
    class_weights = 1.0 / class_counts  # 权重为样本数量的倒数
    ...
    for epoch in range(50):  
        ...
        model.to(device)
        for _, video_batch, labels in pbar:  # 去掉 audio_batch
            _ = _.to(device)
            video_batch = video_batch.to(device)
            labels = labels.to(device)
            optimizer.zero_grad()
            outputs = model(video_batch)  # 只传递 video_batch
        ...
        all_labels = []
        with torch.no_grad():
            for _, video_batch, labels in val_loader:  # 去掉 audio_batch
                outputs = model(video_batch)  # 只传递 video_batch
                loss = criterion(outputs, labels)
                val_loss += loss.item()
        ...
        with torch.no_grad():
            for _, video_batch, labels in train_loader:  # 去掉 audio_batch
                video_batch, labels = video_batch.to(device), labels.to(device)  # 只传递 video_batch
                outputs = model(video_batch)  # 只传递 video_batch
                preds = torch.argmax(outputs, dim=1).cpu().numpy()
                all_preds.extend(preds)
                all_labels.extend(labels.cpu().numpy())
    ...
    all_labels = []
    with torch.no_grad():
        for _, video_batch, labels in test_loader:  # 去掉 audio_batch
            _ = _.to(device)
            video_batch = video_batch.to(device)
            labels = labels.to(device)
            outputs = model(video_batch)  # 只传递 video_batch
