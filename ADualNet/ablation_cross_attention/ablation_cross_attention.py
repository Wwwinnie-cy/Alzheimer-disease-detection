...
class AttentionFusionModel(nn.Module):
    ...
    def forward(self, audio_features, video_features):
        ...
        """
        # 堆叠3层 self-attention，逐层更新 audio_self 和 video_self（cross-attention 已移除）
        for i in range(3):
            # 自注意力机制
        ...
        for i in range(3):
            # 自注意力机制
            audio_attn, _ = self.audio_self_attn[i](audio_self, audio_self, audio_self)
            video_attn, _ = self.video_self_attn[i](video_self, video_self, video_self)
            
            # 添加残差连接
            audio_self = audio_self + audio_attn
            video_self = video_self + video_attn
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model, fold):
        if not os.path.exists('ablation_experiment_V9/ablation_cross_attention/saved_checkpoints_V9'):
            os.makedirs('ablation_experiment_V9/ablation_cross_attention/saved_checkpoints_V9')

        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        model_path = f'ablation_experiment_V9/ablation_cross_attention/saved_checkpoints_V9/checkpoint_fold_{fold}_{timestamp}.pth'
...
log_file = f'ablation_experiment_V9/ablation_cross_attention/patience30_training_log_new2_residual.txt'
