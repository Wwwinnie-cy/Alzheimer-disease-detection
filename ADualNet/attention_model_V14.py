...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model, fold):
        if not os.path.exists('attention_model_V14/saved_checkpoints_V14'):
            os.makedirs('attention_model_V14/saved_checkpoints_V14')

        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        model_path = f'attention_model_V14/saved_checkpoints_V14/checkpoint_fold_{fold}_{timestamp}.pth'
...
log_file = f'V14_patience10_training_log_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
