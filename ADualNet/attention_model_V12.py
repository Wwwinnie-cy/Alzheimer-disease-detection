...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model, fold):
        if not os.path.exists('attention_model_V12/saved_checkpoints_V12'):
            os.makedirs('attention_model_V12/saved_checkpoints_V12')

        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        model_path = f'attention_model_V12/saved_checkpoints_V12/checkpoint_fold_{fold}_{timestamp}.pth'
...
log_file = f'V12_patience30_training_log_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
