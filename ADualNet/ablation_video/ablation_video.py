...
class AttentionFusionModelWithoutVideo(nn.Module):
    def __init__(self, audio_dim, num_heads, num_classes, dropout=0.65):
        super(AttentionFusionModelWithoutVideo, self).__init__()
        ...
        self.fc = nn.Sequential(
            nn.Linear(512, 256),  # 不再拼接视频特征，输入改为512
            nn.ReLU(),
            nn.Dropout(dropout),
    ...
    def forward(self, audio_features):
        ...
        # 特征激活
        audio_final = audio_self.mean(dim=0)

        # 层归一化
        audio_final = self.audio_norm(audio_final)

        # 分类
        logits = self.fc(audio_final)
        return logits
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model, fold):
        if not os.path.exists('ablation_experiment_V9/ablation_video/saved_checkpoints_V9'):
            os.makedirs('ablation_experiment_V9/ablation_video/saved_checkpoints_V9')

        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        model_path = f'ablation_experiment_V9/ablation_video/saved_checkpoints_V9/checkpoint_fold_{fold}_{timestamp}.pth'
...
model = AttentionFusionModelWithoutVideo(audio_dim, num_heads, num_classes).to(device)
...
log_file = f'ablation_experiment_V9/ablation_video/patience30__training_log_new.txt'
...
for fold in range(1, num_folds + 1):
    ...
    for epoch in range(50):  
        ...
        model.to(device)
        for audio_batch, _, labels in pbar:  # 去掉 video_batch
            audio_batch = audio_batch.to(device)
            _ = _.to(device)
            labels = labels.to(device)
            optimizer.zero_grad()
            outputs = model(audio_batch)  # 只传递 audio_batch
        ...
        all_labels = []
        with torch.no_grad():
            for audio_batch, _, labels in val_loader:  # 去掉 video_batch
                audio_batch = audio_batch.to(device)
                _ = _.to(device)
                labels = labels.to(device)
                outputs = model(audio_batch)  # 只传递 audio_batch
        ...
        with torch.no_grad():
            for audio_batch, _, labels in train_loader:  # 去掉 video_batch
                audio_batch, labels = audio_batch.to(device), labels.to(device)  # 只传递 audio_batch
                outputs = model(audio_batch)  # 只传递 audio_batch
                preds = torch.argmax(outputs, dim=1).cpu().numpy()
                all_preds.extend(preds)
                all_labels.extend(labels.cpu().numpy())
    ...
    all_labels = []
    with torch.no_grad():
        for audio_batch, _, labels in test_loader:  # 去掉 video_batch
            outputs = model(audio_batch)  # 只传递 audio_batch
            loss = criterion(outputs, labels)
            test_loss += loss.item()
            preds = torch.argmax(outputs, dim=1).cpu().numpy()
