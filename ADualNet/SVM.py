...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model, fold):
        if not os.path.exists('SVM/saved_checkpoints'):
            os.makedirs('SVM/saved_checkpoints')

        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        model_path = f'SVM/saved_checkpoints/checkpoint_fold_{fold}_{timestamp}.pth'
...
# 日志记录
log_file = f'SVM_training_log_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
...
# 训练和验证函数
def train_model(train_loader, val_loader, feature_extractor, svm, scaler, num_epochs=50, fold=1):
    ...
    for epoch in range(num_epochs):
        ...
        # 训练SVM
        svm.fit(all_train_features, all_train_labels)
        ...
        # 进行SVM预测
        val_preds = svm.predict(all_val_features)
...
def main():
    ...
    for fold in range(1, num_folds + 1):
        ...
        # 使用SVM进行分类
        svm = SVC(kernel='linear', class_weight={0: 0.4, 1: 0.6})
        scaler = StandardScaler()

        fold_conf_matrices, fold_metrics = train_model(train_loader, val_loader, feature_extractor, svm, scaler, num_epochs=num_epochs, fold=fold)
...
# 日志记录
log_file = f'SVM_test_training_log_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
...
def train_model(train_loader, val_loader, feature_extractor, svm, scaler, num_epochs=50, fold=1):
    ...
    for epoch in range(num_epochs):
        ...
        early_stopping(val_loss, svm, fold)
        if early_stopping.early_stop:
            log_message("Early stopping")
            break
