...
class AttentionFusionModel(nn.Module):
    def __init__(self, audio_dim, video_dim, num_heads, num_classes, dropout=0.65):
        ...
        # 堆叠2层 cross-attention (只保留 Audio-to-Video)
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model, fold):
        if not os.path.exists('attention_model_V10/saved_checkpoints_V10'):
            os.makedirs('attention_model_V10/saved_checkpoints_V10')

        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        model_path = f'attention_model_V10/saved_checkpoints_V10/checkpoint_fold_{fold}_{timestamp}.pth'
...
log_file = f'V10_patience30_training_log_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
