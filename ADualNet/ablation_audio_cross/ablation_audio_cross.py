...
class AttentionFusionModel(nn.Module):
    def __init__(self, audio_dim, video_dim, num_heads, num_classes, dropout=0.65):
        ...
        # 堆叠3层 self-attention 和 cross-attention
    ...
    def forward(self, audio_features, video_features):
        ...
        # 堆叠3层 self-attention 和 cross-attention，逐层更新 audio_self 和 video_self
        for i in range(3):
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model, fold):
        if not os.path.exists('ablation_experiment_V9/ablation_audio_cross/saved_checkpoints_V9'):
            os.makedirs('ablation_experiment_V9/ablation_audio_cross/saved_checkpoints_V9')

        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        model_path = f'ablation_experiment_V9/ablation_audio_cross/saved_checkpoints_V9/checkpoint_fold_{fold}_{timestamp}.pth'
...
log_file = f'ablation_experiment_V9/ablation_audio_cross/patience30_training_log_new.txt'
