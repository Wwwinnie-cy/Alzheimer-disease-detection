...
"""改动如下
V12基础上改进，取消test
类别权重的计算：建议根据每个 fold 动态计算类别权重，而不是手动指定固定值。
开tmux的时候10和30搞反了
"""
...
class AttentionFusionModel(nn.Module):
    ...
    def forward(self, audio_features, video_features):
        ...
        # 自注意力机制（1层）
        audio_self, _ = self.audio_self_attn(audio_features, audio_features, audio_features)
        video_self, _ = self.video_self_attn(video_features, video_features, video_features)

        # Video-to-Audio Cross Attention（2层）
        for i in range(2):
            video_cross, _ = self.cross_attn_va[i](video_self, audio_self, audio_self)
            video_self = video_self + video_cross  # 残差连接
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model, fold):
        if not os.path.exists('attention_model_V13/saved_checkpoints_V13'):
            os.makedirs('attention_model_V13/saved_checkpoints_V13')

        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        model_path = f'attention_model_V13/saved_checkpoints_V13/checkpoint_fold_{fold}_{timestamp}.pth'
...
log_file = f'V13_patience10_training_log_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
...
for fold in range(1, num_folds + 1):  
    ...
    # 加载数据
    train_dataset, val_dataset = load_data(fold_path)
    ...
    class_weights = torch.FloatTensor(class_weights).to(device)  # 将权重转换为 tensor
    ...
    for epoch in range(50):   
        device = torch.device('cpu')  
        model.to(device)  # 保留你当前的 .to(device) 调用
        model.train()
        running_loss = 0.0
