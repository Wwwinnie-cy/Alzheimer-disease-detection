...
class AttentionFusionModel(nn.Module):
    def __init__(self, audio_dim, video_dim, num_heads, num_classes, dropout=0.65):
        ...
        # attention layers - 只保留音频主导的 cross-attention
    ...
    def forward(self, audio_features, video_features):
        ...
        # 堆叠3层 self-attention 和 audio主导的 cross-attention，逐层更新 audio_self 和 video_self
        for i in range(3):
            # 自注意力机制
            audio_self, _ = self.audio_self_attn[i](audio_self, audio_self, audio_self)

            # 跨模态交互：以音频为 Query，视频为 Key/Value
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model, fold):
        if not os.path.exists('ablation_experiment_V9/ablation_video_cross/saved_checkpoints_V9'):
            os.makedirs('ablation_experiment_V9/ablation_video_cross/saved_checkpoints_V9')

        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        model_path = f'ablation_experiment_V9/ablation_video_cross/saved_checkpoints_V9/checkpoint_fold_{fold}_{timestamp}.pth'
...
log_file = f'ablation_experiment_V9/ablation_video_cross/patience30_training_log_new.txt'
