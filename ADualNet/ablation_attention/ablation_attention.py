...
class AttentionFusionModel(nn.Module):
    ...
    def forward(self, audio_features, video_features):
        ...
        # 特征直接平均而不是通过 self-attention，确保维度匹配
        audio_self = audio_features.mean(dim=0)  # [batch_size, 512]
        video_self = video_features.mean(dim=0)  # [batch_size, 512]

        # 特征激活和加权融合
        audio_final = self.audio_weight * audio_self
        video_final = self.video_weight * video_self
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model, fold):
        if not os.path.exists('ablation_experiment_V9/ablation_attention/saved_checkpoints_V9'):
            os.makedirs('ablation_experiment_V9/ablation_attention/saved_checkpoints_V9')

        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        model_path = f'ablation_experiment_V9/ablation_attention/saved_checkpoints_V9/checkpoint_fold_{fold}_{timestamp}.pth'
...
log_file = f'ablation_experiment_V9/ablation_attention/patience30_training_log_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
...
for fold in range(1, num_folds + 1):
    ...
    # 初始化优化器
    #model = AttentionFusionModel(audio_dim, video_dim, num_heads, num_classes)
    model = AttentionFusionModel(audio_dim, video_dim, num_heads, num_classes).to(device)
    optimizer = optim.AdamW(model.parameters(), lr=1e-5, weight_decay=1e-5)
