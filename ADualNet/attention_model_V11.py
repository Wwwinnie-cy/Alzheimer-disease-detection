...
class AttentionFusionModel(nn.Module):
    def __init__(self, audio_dim, video_dim, num_heads, num_classes, dropout=0.65):
        ...
        # 两层 cross-attention (Audio-to-Video)
        self.cross_attn_av = nn.ModuleList([nn.MultiheadAttention(embed_dim=512, num_heads=num_heads, dropout=dropout) for _ in range(2)])
    ...
    def forward(self, audio_features, video_features):
        ...
        # Audio-to-Video Cross Attention（2层）
        for i in range(2):
            audio_cross, _ = self.cross_attn_av[i](audio_self, video_self, video_self)
            audio_self = audio_self + audio_cross  # 残差连接
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model, fold):
        if not os.path.exists('attention_model_V11/saved_checkpoints_V11'):
            os.makedirs('attention_model_V11/saved_checkpoints_V11')

        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        model_path = f'attention_model_V11/saved_checkpoints_V11/checkpoint_fold_{fold}_{timestamp}.pth'
...
log_file = f'V11_patience20_training_log_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
